...
function direktt_cf7_send_messages( $contact_form ) {
	...
	if ( $send_to_subscriber && ! empty( $subscriber_message ) && ! empty( $subscription_id ) ) {
		...
		$push_subscriber_message = array(
			'type'    => 'text',
			'content' => $final_subscriber_message,
		);

		Direktt_Message::send_message( array( $subscription_id => $push_subscriber_message ) );
	}
	...
	if ( $send_to_admin && ! empty( $admin_message ) ) {
		...
		if ( ! empty( $subscription_id ) ) {
			...
			$display_name = $user['direktt_display_name'];
...
function direktt_cf7_enqueue_scripts() {
	...
	if ( 'admin.php' === $pagenow && ( 'wpcf7-new' === $page || ( 'wpcf7' === $page && isset( $_GET['post'] ) ) ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Justification: not a form processing, checking if current page is Contact Form 7 page.
